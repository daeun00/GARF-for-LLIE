...
def eval(model, epoch, Peak_psnr, Peak_epoch):
    print("==> Start testing")
    tStart = time.time()
    ...
    if psnr_score > Peak_psnr:
        Peak_psnr = psnr_score
        Peak_epoch = epoch
    return psnr_score, ssim_score, Peak_psnr, Peak_epoch
...
def main(opt, _run):
    ...
    #RGB_v = loss_f.RGB_variation() #초기에 불러오는 작업은 init 변수여서 필요없다. #L_color = loss.L_color()
    #RGB_s = loss_f.RGB_skew()
    #RGB_k = loss_f.RGB_kurt()
    #Intensity_loss = loss_f.Intensity_loss()
    #pix_val_loss = loss_f.RGB_pix_variation()
    ...
    history = {'psnr': [],
               'ssim': []}
    psnr_score, ssim_score, Peak_psnr, Peak_epoch = eval(lighten, 0, Peak_psnr, Peak_epoch)
    print(psnr_score)
    # writer.add_scalar("PSNRtrain", psnr_score)  ##tensorboard
    ...
    for epoch in range(opt.start_iter, opt.nEpochs + 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            ...
            ssim_loss = 1 - ssim(pred_t, NL_t) #NL_t 그라운드 트루스 이미지
            tv_loss = TV_loss(pred_t)
            # mse_loss = Mse_loss(pred_t,NL_t) ###########
            #loss_col = RGB_v(pred_t, NL_t)
            #loss_skew = RGB_s(pred_t, NL_t)
            #loss_kurt = RGB_k(pred_t, NL_t)

            loss = ssim_loss + 0.001 * tv_loss #+ loss_skew + loss_col + 0.001 * loss_kurt #원래꺼
            #loss = ssim_loss + 0.001 * tv_loss + 0.001 * loss_col
            #loss = ssim_loss + 0.001 * tv_loss + 0.001 * mse_loss
            '''
            print(ssim_loss)
            print(tv_loss)
            print(loss_col)
            ...
            if iteration % 10 == 0:
                ...
                logs = {
                    ...
                    #"var_loss": loss_col.data,
                    #"s_loss": loss_skew.data,
                    #"k_loss": loss_kurt.data,
                    #"I_loss": loss_intensity.data,
                    #"pix_var_loss": loss_pix_val.data,
                }
        ...
        #writer.add_scalar("AvgLoss", float(epoch_loss / len(training_data_loader)), epoch)

        if epoch % (opt.snapshots) == 0:
            file_checkpoint = checkpoint(lighten, epoch, opt)
            exp.add_artifact(file_checkpoint)

            psnr_score, ssim_score, Peak_psnr, Peak_epoch = eval(lighten, epoch, Peak_psnr, Peak_epoch)
